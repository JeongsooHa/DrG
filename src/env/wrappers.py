...
def make_env(
	...
	assert mode in {'train', 'color_easy', 'color_hard', 'video_easy', 'video_hard', 'video_hard_train', 'video_hard_test', 'video_driving_train', 'video_driving_test', 'distracting_cs'}, \
	...
	if not is_distracting_cs:
		env = VideoWrapper(env, mode, seed, multi, eval_=eval_)
...
class ColorWrapper(gym.Wrapper):
	...
	def reset(self):
		...
		if 'color' in self._mode:
			...
			if self._mode == 'video_hard' or self._mode == 'video_hard_test' or self._mode == 'video_driving_test':
...
class VideoWrapper(gym.Wrapper):
	...
	def __init__(self, env, mode, seed, multi, eval_=None):
		gym.Wrapper.__init__(self, env)
		self._mode = mode
		self._seed = seed
		self._random_state = np.random.RandomState(seed)
	...
	def _get_video_paths(self):
		...
		if 'video_easy' in self._mode:
			...
			# self._video_paths = [os.path.join(video_dir, 'test', video_) for video_ in os.listdir(os.path.join(video_dir, 'test'))]
		elif 'video_hard' in self._mode:
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(100)]
		elif 'video_hard_train' in self._mode:
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(0,30)]
		elif 'video_hard_test' in self._mode:
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(33,100)]
		elif 'video_driving_train' in self._mode:
			self._video_paths = [os.path.join(video_dir, video_) for video_ in os.listdir(os.path.join(video_dir))]
		elif 'video_driving_test' in self._mode:
			self._video_paths = [os.path.join(video_dir, video_) for video_ in os.listdir(os.path.join(video_dir))]
		else:
	...
	def _load_video(self, video):
		...
		if not cap.get(cv2.CAP_PROP_FRAME_WIDTH) >= 100 or not cap.get(cv2.CAP_PROP_FRAME_HEIGHT) >= 100:
			return None
	...
	def _reset_video(self):
		while True:
			if self._multi:
				self._index = (self._seed+self._index) % self._num_videos
			else:
				self._index = (self._index + 1) % self._num_videos
			self._index = (random.randint(0, len(self._video_paths)-1))
			if self._eval is not None:
				self._index = self._eval
			self._index = 8
